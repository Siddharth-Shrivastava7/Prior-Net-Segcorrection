...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes=1000, output_stride=8, width_mult=1.0, inverted_residual_setting=None, round_nearest=8, num_ip_channels = 3):
        """
        ...
        # features = [ConvBNReLU(3, input_channel, stride=2)] # changing (not using for experiments)
        features = [ConvBNReLU(num_ip_channels, input_channel, stride=2)]
        current_stride *= 2
        dilation=1
        previous_dilation = 1
...
def mobilenet_v2(pretrained=False, progress=True, num_ip_channels=3, **kwargs):
    """
    ...
    """
    model = MobileNetV2(num_ip_channels=num_ip_channels, **kwargs)
    if pretrained:
        ...
        # new_state_dict={k:v if v.size()==model_dict[k].size()  else  model_dict[k] for k,v in zip(model_dict.keys(), pretrained_dict.values())}
        ...
        # model.load_state_dict(new_state_dict, strict=False) # 3. load the new state dict
