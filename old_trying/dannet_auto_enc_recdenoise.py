...
def init_model(cfg): 
    model_reconst = reconst_auto_encoder(cfg.rec_num_ip_channels, cfg.rec_num_out_channels, cfg.rec_base_channels).cuda() 
    model_denoise = denoise_auto_encoder(cfg.den_num_ip_channels, cfg.den_num_out_channels, cfg.den_base_channels).cuda() 
...
class Trainer(BaseTrainer):
    ...
    def iter(self, batch):
        ...
        ## perturbed gt label with exchanges from the seg prediction 
        seg_pred = torch.argmax(seg_pred, dim=1)               
        for _ in range(self.config.num_patch_perturb): # number of patches to be cropped
            randx = np.random.randint(seg_pred.shape[1]-self.config.patch_size)
            randy = np.random.randint(seg_pred.shape[2]-self.config.patch_size)   
    ...
    def train_rec_iter(self, batch): 
        _, seg_label, name = batch
        ## converting the gt seglabel with train ids into onehot matrix 
        seg_label = [seg_label[bt] for bt in range(seg_label.shape[0])]  
        for label in seg_label:
            label[label==255] = 19 
        ...
        gt_onehot_batch = gt_onehot_batch[:, :19, :,:]   
        label_gt_tensor = gt_onehot_batch.detach().clone()  
        rec_gt_label = self.model_rec(label_gt_tensor.float().cuda())  
        
        loss = nn.MSELoss() 
        rec_mse_loss = loss(rec_gt_label, label_gt_tensor) 
        self.losses.rec_mse_loss = rec_mse_loss 
        rec_mse_loss.backward()
    
    def train_den_iter(self, batch): 
        image, seg_label, name = batch 
        # to form this net and do the inference part ... 
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.config.epochs)):
            ...
            for i_iter, batch in tqdm(enumerate(self.loader)):
                ...
                train_rec_iter_loss = 0  
                # training  the gt reconstruction network 
                for iter in range(self.config.rec_max_iter_train):
                    self.model_rec = self.model_rec.train()
                    self.model_den = self.model_den.eval() 
                    
                    self.optim.zero_grad()
                    self.train_rec_iter(batch) 
                    train_rec_iter_loss += self.losses['rec_mse_loss'].item()
                    self.optim.step() 
                    print('train_per_rec_iter_loss:', self.losses['rec_mse_loss'].item())
                train_rec_iter_loss /= len(range(self.config.rec_max_iter_train))        
                
                # training the denoising network
                train_den_iter_loss = 0  
                for iter in range(self.config.den_max_iter_train):
                    self.model_rec = self.model_rec.eval()
                    self.model_den = self.model_den.train() 
                    self.optim.zero_grad()
                    self.train_den_iter(batch)  
                    train_den_iter_loss += self.losses['rec_mse_loss'].item()
                    self.optim.step() 
                    print('train_per_rec_iter_loss:', self.losses['rec_mse_loss'].item())
