...
def init_model(cfg): 
    model = auto_encoder(cfg.num_ip_channels, cfg.num_class, multi_channels=cfg.multi_channels_rec).cuda()  
...
class BaseTrainer(object):
    ...
    def validate(self, epoch): 
        ...
        for i_iter, batch in tqdm(enumerate(testloader)):
            ...
            gt_label_tensor = gt_onehot_batch.detach().clone() 

            seg_pred = self.model(gt_label_tensor.float().cuda()) 
