        ...
        try:  
            if self.dataset in ['acdc_train_label', 'acdc_val_label']: 
                ...
                if self.set=='val': 
                    ...
                    if self.cfg.method_train == 'perturb':  
                    ...
                    elif self.cfg.method_train == 'perturb_discrete_patches': 
                    ...
                    elif self.cfg.method_train == 'perturb_both':  

                        label = label_perturb ## saving the original
                    ...
                    #============#===========random saving====++++++++++++++++++++
                    # seg_preds = torch.argmax(label_perturb_tensor, dim=2) 
                    # seg_preds[label == 255] = 19
                    # img_color = label_img_to_color(seg_preds, 'trial.png') 
                    # print('saved')
    ...
    def validate(self, epoch): 
        ...
        for i_iter, batch in tqdm(enumerate(testloader)):
            ...
            if self.config.rgb_save:  
                ...
                seg_preds = [torch.tensor(label_img_to_color(seg_preds[sam], 'save_patch_ip/' + str(sam + it) + '.png')) for sam in range(seg_pred.shape[0])]  

                seg_preds = torch.argmax(seg_pred, dim=1) 
                seg_preds = [torch.tensor(label_img_to_color(seg_preds[sam], 'save_patch/' + str(sam + it) + '.png')) for sam in range(seg_pred.shape[0])]  
