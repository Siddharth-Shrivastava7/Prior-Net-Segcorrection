...
class Trainer(BaseTrainer):
    def __init__(self, model, args):
        ...
        if self.args.weighted_ce and self.args.ignore_classes: 
            ...
            self.loss = CrossEntropy2d(ignore_classes=args.ignore_classes, weights=weights) 
            ...
            self.loss = nn.CrossEntropyLoss(ignore_index=255, weight=weights) 
        elif self.args.ignore_classes:
            self.loss = CrossEntropy2d(ignore_classes=args.ignore_classes)  
        else: 
    ...
    def iter(self, batch): 
        ...
        seg_loss = self.loss(seg_pred, seg_label)        
    ...
    def train(self):
        ...
        writer.export_scalars_to_json("/HOME/SIDD_S/PRIOR-NET-SEGCORRECTION/all_scalars.json")
        writer.close() 
