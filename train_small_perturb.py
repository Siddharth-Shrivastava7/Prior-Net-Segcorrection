...
parser.add_argument("--patch_size", type=int, default=20, # may be req 
...
parser.add_argument("--end_learning_rate", type=float, default=1e-4, # may be req  
...
parser.add_argument("--img_size", type=int, default=768,  # may be req 
...
parser.add_argument("--num_masks", type=int, default=1000)   # may be req  
...
def grp_perturb_gt_gen(mask_paths, gt_path, pred_path):
    gt = Image.open(gt_path)  
    pred = Image.open(pred_path) 
    gt = np.array(gt)
    pred = np.array(pred)
    ...
    for mask_path in mask_paths:
        ...
        randx = np.random.randint(gt.shape[0]-args.patch_size)
        randy = np.random.randint(gt.shape[1]-args.patch_size) 
        big_mask[randx: randx+args.patch_size, randy: randy+args.patch_size] = mask  
...
class BaseDataSet(data.Dataset): 
    ...
    def __getitem__(self, index): 
        ...
        try:
            if self.set == 'val':  
                # for every different perturbation this could work since we made separate offline folder for each perturbation
                ...
                if 'dannet' in args.val_perturb_path:  
                    ...
                    for i in range(args.num_masks): 
                        ...
                        mask_paths.append(os.path.join(args.mask_main_path, self.mask_lst[rand])) 
                    per_gt = grp_perturb_gt_gen(mask_paths, gt_path, pred_path = datafiles['img'])      
                    image = np.array(transforms_compose_label(per_gt)) # for resizing it to desired size
                elif 'patch' in args.val_perturb_path: 
                    ...
                    image = label_img_to_color(label_perturb) 
                else:
                    for _ in range(self.args.num_pix_perturb): 
...
class Trainer(BaseTrainer):
    ...
    def train(self):
        ...
        train_epoch_loss = 0 
        max_decay_steps = self.args.max_iters  # maximum number of iterations
