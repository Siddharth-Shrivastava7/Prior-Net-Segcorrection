...
parser.add_argument('--load_val_path', type = str, help='loading path for perturbed validation images', default='random_small_shapes_overlap')  
...
class BaseDataSet(data.Dataset): 
    def __init__(self, args, root, list_path,dataset, num_class, ignore_label=255, set='val'):
        ...
        self.fixed = 0
        ## mask paths 
        self.per_gt_val_path = os.path.join('/home/sidd_s/scratch/dataset/acdc_trainval/rgb_anon/night/synthetic/val', args.load_val_path)
            ...
            if self.dataset == 'acdc_train_label' or self.dataset == 'acdc_val_label': 
                ...
                if self.args.random_bin_augment and (not self.set == 'val'):
                    ...
                    for i in range(args.num_masks): 
                        ...
                        mask_paths.append(os.path.join(args.mask_main_path, mask_lst[rand])) 
                    per_gt = perturb_gt_gen(mask_paths, gt_path, pred_path = datafiles['img'])      
                    image = per_gt 
                elif self.args.random_bin_augment and (self.set == 'val'): 
                    # fixed images validation
                    nme = name.split('/')[-1] 
                    path = os.path.join(self.per_gt_val_path, nme)
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.args.epochs)):
            ...
            if (valid_epoch_loss > worst_val_epoch_loss):
                ...
                wname = self.args.save_writer_name + '_worst_'+ str(epoch) +'.pth' # for the ce loss  
            ...
            if(valid_epoch_loss < best_val_epoch_loss):
                ...
                print('********************') 
                print("MODEL UPDATED")
                name = self.args.save_writer_name + str(epoch) +'.pth' # for the ce loss  
